...
bool platform_init(int width, int height, const std::string& title) {
	...
	if (!RegisterClassEx(&wc)) {
		RTGJ_LOG_CRITICAL("platform_init: RegisterClassEx failed (error %lu)",
		                  GetLastError());
		return false;
	}
	...
	g_window = CreateWindowEx(0, WINDOW_CLASS_NAME, title.c_str(), style,
	                          CW_USEDEFAULT, CW_USEDEFAULT, outer_w, outer_h,
	                          nullptr, nullptr, g_instance, nullptr);
	if (!g_window) {
		RTGJ_LOG_CRITICAL("platform_init: CreateWindowEx failed (error %lu)",
		                  GetLastError());
		return false;
	}
